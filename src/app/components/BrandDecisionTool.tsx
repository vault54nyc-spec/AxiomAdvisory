...
export function BrandDecisionTool() {
  const [selections, setSelections] = useState<Record<DecisionCategoryId, 1 | 2 | 3 | null>>({
    voice: null,
    palette: null,
    logo: null,
    tagline: null,
  ...
  const exportSummary = () => {
    const lines = [
      ...
      "=".repeat(45),
      "",
      `Brand Direction: ${getSummaryValue("voice")}`,
      `Color Palette: ${getSummaryValue("palette")}`,
      `Logo Concept: ${getSummaryValue("logo")}`,
      `Tagline: ${getSummaryValue("tagline")}`,
      `Website Layout: ${getSummaryValue("layout")}`,
      `Service Structure: ${getSummaryValue("tier")}`,
      `GTM Strategy: ${getSummaryValue("gtm")}`,
      `Content Pillar: ${getSummaryValue("content")}`,
      "",
